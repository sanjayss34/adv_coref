# This file is a modification of coref_model.py from https://github.com/kentonl/e2e-coref
...
class CorefModel(object):
  ...
  def restore(self, session):
    ...
    # checkpoint_path = os.path.join("/scratch/sanjay/pretrained2018/final/", "model.max.ckpt")
    checkpoint_path = os.path.join("/scratch/sanjay/logs_new17/final/", "model.max.ckpt")
  ...
  def get_predictions_and_loss(self, tokens, context_word_emb, head_word_emb, lm_emb, char_index, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids):
    ...
    if not self.lm_file:
      ...
      if self.config["char_embedding_size"] > 0:
        ...
        with tf.variable_scope("char_convolution", reuse=tf.AUTO_REUSE):
      ...
      candidate_span_emb_orig, candidate_head_scores = self.get_span_emb(flattened_head_emb, context_outputs, candidate_starts, candidate_ends) # [num_candidates, emb]
    ...
    def compute_from_emb(candidate_span_emb):
      with tf.variable_scope("prediction_scope", reuse=tf.AUTO_REUSE):
        ...
        self.head_scores = tf.gather(candidate_head_scores, top_span_indices) # [k, max_span_width]
    ...
    perturb_span_emb = normalize_vec(span_emb_grad, self.config['adv_perturb_length'])
    _, adv_loss = compute_from_emb(candidate_span_emb_orig+perturb_span_emb)
    # _, adv_loss = compute_from_emb(candidate_span_emb_orig+delta)
    return predictions, (1-self.config['adv_alpha'])*loss+self.config['adv_alpha']*adv_loss
  ...
  def get_span_emb(self, head_emb, context_outputs, span_starts, span_ends):
    ...
    return span_emb, tf.squeeze(span_head_scores) # [k, emb], [k, max_span_width]
