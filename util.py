# This file is a modification of util.py from https://github.com/kentonl/e2e-coref
...
def initialize_from_env():
  ...
  config_filename = 'experiments_adv.conf'
  # if len(sys.argv) >= 3:
  #   config_filename = sys.argv[2]+config_filename
  config = pyhocon.ConfigFactory.parse_file(config_filename)[name]
...
def dropout_with_seed(x, keep_prob, seeds):
    rands = tf.contrib.stateless.stateless_random_uniform(tf.shape(x), seeds)
    xnew = tf.where(tf.less(rands, keep_prob*tf.ones_like(x, dtype=tf.float32)), x/keep_prob, tf.zeros_like(x, dtype=tf.float32))
    return xnew
...
def ffnn(inputs, num_hidden_layers, hidden_size, output_size, dropout, output_weights_initializer=None, dropout_seed=None):
  ...
  for i in range(num_hidden_layers):
    ...
    if dropout is not None:
      ...
      # current_outputs = dropout_with_seed(current_outputs, dropout, dropout_seed)
